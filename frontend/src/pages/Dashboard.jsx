...
export const Dashboard = () => {
  ...
  const plTeams = teams.filter(t => t.leagueId === 'pl').sort((a, b) => a.rank - b.rank).slice(0, 4);

  const isLoading = loadingMatches || loadingNews || loadingTransfers;
        ...
        <div className="flex gap-3 items-center">
          ...
          <div>
            ...
            <p className="text-xs text-muted max-w-xl">
              Arsenal takes on Manchester City today in the match of the season. Haaland leads goals with 28. Real Madrid closes in on Zubimendi.
            </p>
          </div>
        </div>
        <Button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-display font-extrabold text-sm text-text flex items-center gap-1.5">
                <Calendar size={14} className="text-muted" /> Today's Match Matrix
              </h3>
              <Link to="/fixtures" className="text-xs text-primary hover:underline flex items-center gap-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {news.slice(0, 2).map(n => (
                <Card key={n.id} onClick={() => navigate('/news')} className="cursor-pointer hover:border-primary/20 transition-all flex gap-4 p-4 items-start">
                  <img src={n.image} alt={n.title} className="w-16 h-16 object-cover rounded-lg shrink-0 border border-border" />
                  <div className="space-y-1 min-w-0">
                    <span className="text-[9px] text-primary font-bold uppercase">{n.date}</span>
                    <h4 className="font-semibold text-text text-xs leading-snug line-clamp-2">{n.title}</h4>
                    <p className="text-[10px] text-muted line-clamp-1">{n.summary}</p>
                  </div>
                </Card>
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-4 border border-border">
            <div className="flex justify-between items-center mb-3">
              ...
              <Link to="/league/pl" className="text-[10px] text-primary hover:underline">Full Table</Link>
            </div>
            <div className="space-y-2">
              ...
              {plTeams.map((team, idx) => (
                ...
                >
                  ...
                  <span className="text-center text-[10px] font-medium text-muted">{team.played}</span>
                  <span className="text-center text-[10px] font-medium text-muted">+{team.gd}</span>
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-text truncate leading-tight">{player.name}</p>
                      <p className="text-[9px] text-muted">{player.teamName} • {player.position}</p>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="font-display font-extrabold text-xs text-primary">{player.stats.goals}</span>
          ...
          <div>
            <div className="flex justify-between items-center mb-2.5">
              ...
              <Link to="/transfers" className="text-xs text-primary hover:underline flex items-center gap-1">
